...
TASK_FILE="$WATCH_DIR/zhilian_list_tasks.jsonl"
RESULTS_FILE="$WATCH_DIR/zhilian_list_results.jsonl"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ARCHIVE_SCRIPT="$SCRIPT_DIR/archive_outputs.py"
PARSE_SCRIPT="$SCRIPT_DIR/parse_queue_results.py"
STATE_DIR="/tmp/watch_job_list"
...
" | while read -r line; do log "RETRY: $line"; done
	...
	if [ "$phase" = "notifying" ]; then
		...
		if [ "$now" -ge "$next_notify" ] && [ "$attempt" -lt "$MAX_RETRY" ]; then
			...
			result=$(notify_popup "Jobsniper List Pipeline Done" "$msg" "$attempt")
			case "$result" in
				confirmed)
