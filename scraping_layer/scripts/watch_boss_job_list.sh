...
PARSE_SCRIPT="$SCRIPT_DIR/parse_queue_results.py"
STATE_DIR="/tmp/watch_boss_job_list"
...
if [ -f "$LOCK_FILE" ]; then
	...
	if kill -0 "$old_pid" 2>/dev/null; then
		echo "List monitor is already running (pid $old_pid)"
		exit 1
	fi
fi
echo $$ > "$LOCK_FILE"
...
log "========== LIST PIPELINE MONITOR START =========="
...
" | while read -r line; do log "RETRY: $line"; done
...
" | while read -r line; do log "RETRY: $line"; done
	...
	if [ "$phase" = "archiving" ]; then
		...
		log "ARCHIVE: $kw"
		: > "$ARCHIVE_ERR"
		python3 "$ARCHIVE_SCRIPT" "$kw" --platform boss --since-minutes 120 > "$ARCHIVE_JSON" 2> "$ARCHIVE_ERR"
		archive_rc=$?
		...
		moved=$(python3 -c "import json; print(json.load(open('$ARCHIVE_JSON')).get('moved_count',0))" 2>/tmp/watch_job_list/archive_parse.err || echo 0)
		if [ -s /tmp/watch_job_list/archive_parse.err ]; then
			while IFS= read -r line; do log "ARCHIVE_PARSE_ERR: $line"; done < /tmp/watch_job_list/archive_parse.err
		fi
	...
	if [ "$phase" = "notifying" ]; then
		...
		if [ "$now" -ge "$next_notify" ] && [ "$attempt" -lt "$MAX_RETRY" ]; then
			...
			msg="List pipeline complete: done=$(get_status "done") failed=$(get_status "failed") total=$(get_status "total")"
			log "POPUP: $attempt/$MAX_RETRY"
			result=$(notify_popup "JobSniper BOSS List Pipeline Done" "$msg" "$attempt")
			case "$result" in
				confirmed)
					...
					cleanup; exit 0 ;;
				cancelled)
					log "CANCELLED, will retry later"
					set_status "next_notify" $(( now + 120 )) ;;
				timeout)
					log "TIMEOUT, will retry"
					set_status "next_notify" $(( now + 60 )) ;;
			esac
		fi
	fi
