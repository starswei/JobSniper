...
TASK_FILE="$WATCH_DIR/boss_detail_tasks.jsonl"
RESULTS_FILE="$WATCH_DIR/boss_detail_results.jsonl"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ARCHIVE_SCRIPT="$SCRIPT_DIR/archive_outputs.py"
PARSE_SCRIPT="$SCRIPT_DIR/parse_detail_results.py"
STATE_DIR="/tmp/watch_boss_downloads"
...
采集已暂停。请在 Chrome 中完成人机验证后，再触发详情队列唤醒/恢复，然后重新启动监视脚本。"
		notify_popup "JobSniper BOSS Captcha Detected" "$msg" "1" >/dev/null
		cleanup; exit 2
	fi
...
" | while read line; do log "RETRY: $line"; done
	...
	if [ "$phase" = "notifying" ]; then
		...
		if [ "$now" -ge "$next_notify" ] && [ "$attempt" -lt "$MAX_RETRY" ]; then
			...
			result=$(notify_popup "JobSniper BOSS Pipeline Done" "$msg" "$attempt")
			case "$result" in
				confirmed)
